...
| [`zero-bloat-icon-favicon-skill`](skills/zero-bloat-icon-favicon-skill/SKILL.md) | Generator favicon, touch icon, dan web asset berstandar ganda: Cupertino Crystal Glass (`skill.megapass.web.id`) dan Minimalist Flat (`pastree.megapass.web.id`): 10 preset Lucide scale-2, highlight bevel, slicing C-native 11 aset (ICO 16/32/48/64, Retina 48/96, Apple 180, PWA 192/512), self-test bit-for-bit, zero Node.js. | `favicon`, `icon`, `touch icon`, `app icon`, `generate gambar icon`, `webmanifest` |
